"""
pipeline_api_json_sgolla.py - Module 4 Script
...
Run from root project folder with:
  ...
  uv run python -m nlp.pipeline_api_json_sgolla
"""
...
def main() -> None:
    ...
    LOG.info(f"EXTRACT complete: {len(json_data)} records written to {RAW_JSON_PATH}")

    LOG.info("BEGIN VALIDATE stage")
    # VALIDATE
    validated_data = run_validate(
        json_data=json_data,
        LOG=LOG,
    )
    LOG.info(f"VALIDATE complete: {len(validated_data)} records passed validation")

    LOG.info("BEGIN TRANSFORM stage")
    # TRANSFORM
    df = run_transform(
    ...
    LOG.info(
        f"TRANSFORM complete: DataFrame has {df.height} rows and {df.width} columns"
    )

    LOG.info("BEGIN LOAD stage")
    # LOAD
    run_load(
        df=df,
    ...
    LOG.info(f"LOAD complete: file written to {PROCESSED_CSV_PATH}")
